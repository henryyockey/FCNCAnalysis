...
for key in dir_list:
    for S in blackList:
        if S in key:
            break
    if 'data' in key and 'DoubleEG' in key:
        a,b,c,d = key.split("_")   
        DATASAMPLES[key] = [    [],    "data",    d,    "2017",    c,    "1",    "41.53",    "1"]
    else:
	if 'DY50' in key:
            DATASAMPLES[key] = [    [],    "mc",    "none",    "2017",    "none",    "1",    "41.53",    "0"]
...
#        if 'data' in root and 'DoubleEG' in root:
#            for key, value in DATASAMPLES.items():
#                if key in root:
#                    value[0].append(root[19:])
