...
//      "LLFoffZm", "LFFoffZm","FFFoffZm"
   ...
   std::vector<TString> particles {"TopA", "TopB", "W", "Z"};
   ...
   Dim2EFT Hists_rjr = makeHistsEFT(particles, vars_rjr);
...
*/
         ...
         if (cat == "3LonZ" && nbjet==1 && njet>1)
         {
            ...
            for (int p=0; p<4; p++)
            {
               ...
               DecayRecoFrame * recoP;
               if (particle == "TopA"){
                  recoP = jigsaw_ttFCNC->Ta;
               } else if (particle=="TopB") {
                  recoP = jigsaw_ttFCNC->Tb;
               } else if (particle=="W") {
                  recoP = jigsaw_ttFCNC->W;
               } else if (particle=="Z") {
                  recoP = jigsaw_ttFCNC->Z;
               }

               float cosDPhi = cos(recoP->GetDeltaPhiDecayAngle());
               Hists_rjr[p][vInd(vars_rjr,"Mass")]->Fill(recoP->GetFourVector().M(), weight);
               Hists_rjr[p][vInd(vars_rjr,"Pt")]->Fill(recoP->GetFourVector().Pt(), weight);
               Hists_rjr[p][vInd(vars_rjr,"Eta")]->Fill(recoP->GetFourVector().Eta(), weight);
               Hists_rjr[p][vInd(vars_rjr,"Phi")]->Fill(recoP->GetFourVector().Phi(), weight);
               Hists_rjr[p][vInd(vars_rjr,"CosDecayAngle")]->Fill(cosDPhi, weight);
            } 
