...
void MyAnalysis::Loop(TString fname, TString data, TString dataset ,string year, TString runTemp, float xs, float lumi, float Nevent)
{
...
//   Dim4 Hists_2l_fakes = makeHists(regions_2l_fakes, channels, regions, vars);
   ...
   // cut table
   const std::map<TString, std::vector<float>> cut_vars = 
   {
//      {"TriggerCutTable",     		{0, 	16, 	0, 	16}},
      {"ElectronSelectionCutTable",	{0, 	 8,	0,	 8}},
//      {"MuonSelectionCutTable",		{2,	 4,	0, 	 4}},
//      {"EventCategorizationTable",	{3, 	14, 	0, 	14}},
   };

   Dim1 Hists_cut_tables = makeHists(cut_vars);

   const std::map<TString, std::vector<float>> region_counts =
   {
      {"RegionCounts", 			{0,    	3,     	0,	3}},
      {"nbJetsEpEm",			{1, 	6,	0,	6}},
      {"nbJetsMUpMUm",                  {2,     6,      0,      6}},
      {"nbJetsEpmMUmp",                 {3,     6,      0,      6}},
   };

//   Dim1 HistsRegions = makeHists(region_counts);
...
/*******************************************************************************************/
	 ...
	 isTight = tightSelElec(isFO, Electron_mvaTTHUL[i]) && isLoose;
         ...
         for (int i=0; i<8; i++) {
            if (eCuts[i]) Hists_cut_tables[0]->Fill(i);
         }
	 ...
	 if ((**it).lep_==1)  {
      ...
      for (jet_candidate *jet : *selectedJets) {
         ...
         if (eventID=="1:27131:27130105") {
            cout << "Jet Flavor: " << jet->flavor_ <<"\tJet is btagged: " << jet->btag_<< "\tJet pt: " << jet->pt_ << "\tJet eta: " << jet->eta_ << endl;
         }
         // b-quark
         if (abs(jet->flavor_)==5) {
            P_bjet_mc *= (jet->btag_) ? (BJetEff) : (1 - BJetEff); 
            P_bjet_data *= (jet->btag_) ? (BJetEff*BJetSF) : (1 - BJetEff*BJetSF);
            if (eventID=="1:27131:27130105") cout << "Efficiency: " << BJetEff << "\tScaleFactor: " << BJetSF << endl;
         }
         // c quark
         else if (abs(jet->flavor_)==4) {
            P_bjet_mc *= (jet->btag_) ? (CJetEff) : (1 - CJetEff);
            P_bjet_data *= (jet->btag_) ? (CJetEff*CJetSF) : (1 - CJetEff*CJetSF);
            if (eventID=="1:27131:27130105") cout << "Efficiency: " << CJetEff << "\tScaleFactor: " << CJetSF << endl;
         ...
         // light quark / gluon
            ...
            if (eventID=="1:27131:27130105") cout << "Efficiency: " << LJetEff << "\tScaleFactor: " << LJetSF << endl;
...
//      Hists_cut_tables[3]->Fill(fakeChannel);
...
//         if(eventID=="299329:92:157045400") cout << cat << endl;
         int nbJets = 0;
	 ...
	 mll = ((*selectedLeptons)[0]->p4_+(*selectedLeptons)[1]->p4_).M();
         ...
         Float_t llPt = ((*selectedLeptons)[0]->p4_+(*selectedLeptons)[1]->p4_).Pt();
	 Float_t llDr = deltaR((*selectedLeptons)[0]->eta_,(*selectedLeptons)[0]->phi_,(*selectedLeptons)[1]->eta_,(*selectedLeptons)[1]->phi_);
	 Float_t llDPhi = abs(deltaPhi((*selectedLeptons)[0]->phi_,(*selectedLeptons)[1]->phi_));
         ...
         if (ch != -1) {
	      weight*=eleNominalWeight*muNominalWeight*fakefactor*P_bjet_data/P_bjet_mc;
              ...
              if (cat=="EpmMUmp" && reg ==0){
               sfsum +=  P_bjet_data/P_bjet_mc;	      
               sum++;
               cout << P_bjet_data/P_bjet_mc << endl;
              }
              Hists[ch][reg][vInd(vars,"lep1Pt")]->Fill((*selectedLeptons)[0]->pt_, weight);
              Hists[ch][reg][vInd(vars,"lep1Pt200")]->Fill((*selectedLeptons)[0]->pt_, weight);
	      Hists[ch][reg][vInd(vars,"lep1Eta")]->Fill((*selectedLeptons)[0]->eta_, weight);
              Hists[ch][reg][vInd(vars,"lep1Phi")]->Fill((*selectedLeptons)[0]->phi_, weight);
              Hists[ch][reg][vInd(vars,"lep2Pt")]->Fill((*selectedLeptons)[1]->pt_, weight);
              Hists[ch][reg][vInd(vars,"lep2Eta")]->Fill((*selectedLeptons)[1]->eta_, weight);
              Hists[ch][reg][vInd(vars,"lep2Phi")]->Fill((*selectedLeptons)[1]->phi_, weight);
              Hists[ch][reg][vInd(vars,"llM")]->Fill(mll, weight);
              Hists[ch][reg][vInd(vars,"llPt")]->Fill(llPt, weight);
              Hists[ch][reg][vInd(vars,"llDr")]->Fill(llDr, weight);
              Hists[ch][reg][vInd(vars,"llDphi")]->Fill(llDPhi, weight);
              Hists[ch][reg][vInd(vars,"njet")]->Fill(selectedJets->size(), weight);
              Hists[ch][reg][vInd(vars,"nbjet")]->Fill(nbJets, weight);
              Hists[ch][reg][vInd(vars,"Met")]->Fill(MET_pt, weight);
              Hists[ch][reg][vInd(vars,"MetPhi")]->Fill(MET_phi, weight);
              Hists[ch][reg][vInd(vars,"nVtx")]->Fill(PV_npvs, weight);
              Hists[ch][reg][vInd(vars,"llMZw")]->Fill(mll, weight);
	      if (selectedJets->size()>0) {
		 Hists[ch][reg][vInd(vars,"jet1Pt")]->Fill((*selectedJets)[0]->pt_, weight);
                 Hists[ch][reg][vInd(vars,"jet1Eta")]->Fill((*selectedJets)[0]->eta_, weight);
                 Hists[ch][reg][vInd(vars,"jet1Phi")]->Fill((*selectedJets)[0]->phi_, weight);
	      }
	      // Fill all bjet reg
              Hists[ch][3][vInd(vars,"lep1Pt")]->Fill((*selectedLeptons)[0]->pt_, weight);
              Hists[ch][3][vInd(vars,"lep1Pt200")]->Fill((*selectedLeptons)[0]->pt_, weight);
              Hists[ch][3][vInd(vars,"lep1Eta")]->Fill((*selectedLeptons)[0]->eta_, weight);
              Hists[ch][3][vInd(vars,"lep1Phi")]->Fill((*selectedLeptons)[0]->phi_, weight);
              Hists[ch][3][vInd(vars,"lep2Pt")]->Fill((*selectedLeptons)[1]->pt_, weight);
              Hists[ch][3][vInd(vars,"lep2Eta")]->Fill((*selectedLeptons)[1]->eta_, weight);
              Hists[ch][3][vInd(vars,"lep2Phi")]->Fill((*selectedLeptons)[1]->phi_, weight);
              Hists[ch][3][vInd(vars,"llM")]->Fill(mll, weight);
              Hists[ch][3][vInd(vars,"llPt")]->Fill(llPt, weight);
              Hists[ch][3][vInd(vars,"llDr")]->Fill(llDr, weight);
              Hists[ch][3][vInd(vars,"llDphi")]->Fill(llDPhi, weight);
              Hists[ch][3][vInd(vars,"njet")]->Fill(selectedJets->size(), weight);
              Hists[ch][3][vInd(vars,"nbjet")]->Fill(nbJets, weight);
              Hists[ch][3][vInd(vars,"Met")]->Fill(MET_pt, weight);
              Hists[ch][3][vInd(vars,"MetPhi")]->Fill(MET_phi, weight);
              Hists[ch][3][vInd(vars,"nVtx")]->Fill(PV_npvs, weight);
              Hists[ch][3][vInd(vars,"llMZw")]->Fill(mll, weight);
              if (selectedJets->size()>0) {
                 Hists[ch][3][vInd(vars,"jet1Pt")]->Fill((*selectedJets)[0]->pt_, weight);
                 Hists[ch][3][vInd(vars,"jet1Eta")]->Fill((*selectedJets)[0]->eta_, weight);
                 Hists[ch][3][vInd(vars,"jet1Phi")]->Fill((*selectedJets)[0]->phi_, weight);
              }

            for (int i=0; i<nElectron; i++) {
               Hists[ch][reg][vInd(vars,"eTC")]->Fill(Electron_tightCharge[i]);
               Hists[ch][3][vInd(vars,"eTC")]->Fill(Electron_tightCharge[i]);
            }
