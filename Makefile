...
INCLUDES       = -I./include -I$(EFT) -I$(BOOST) -I$(CVMA) -I./RestFrames/include
...
SRCS = src/BTagCalibrationStandalone.cc src/RoccoR.cc src/lepton_candidate.cc src/jet_candidate.cc src/PU_reWeighting.cc src/MyAnalysis.cc src/objects.cc src/histhelper.cc src/scalefactor.cc src/selection.cc src/XGBHelper.cc src/JigsawRecTZFCNC.cc
OBJS =  $(patsubst %.C,%.o,$(SRCS:.cc=.o))

LIB=lib/libmain.so


.SUFFIXES: .cc,.C,.hh,.h,.hpp
...
bin/RunAll : src/main.C  $(LIB)
	mkdir -p bin
	$(CXX) $(CXXFLAGS) -ldl -L./lib -lCondFormatsJetMETObjects -lCondFormatsSerialization -lcorrectionlib -lEFTGenReaderEFTHelperUtilities -L./RestFrames/lib -lRestFrames -lmain src/main.C $(GLIBS) -o $@
