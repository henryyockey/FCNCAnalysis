...
def stackPlotsBDT(hists, SignalHists, Fnames, ch = "channel", reg = "region", year='2017', var="sample", varname="v",wc="wc", density=False):
...
#        print(type(hists[num]))
    ...
    name = year+ch+reg+var+wc
    canvas = ROOT.TCanvas(name,name,50,50,865,780)
    canvas.SetGrid();
    ...
    dummy.SetMarkerStyle(ROOT.kFullCircle)
    ...
    Label_training = ROOT.TLatex(0.2,0.75, var+" trained / " + wc + " weighted")
    Label_training.SetNDC()
    Label_training.SetTextFont(42)
    Label_training.Draw("same") 
...
#        Label_DM.Draw("same")
    ...
    dummy_ratio.Draw()
    canvas.Print(year + '/' + ch +'/'+reg+'/'+var+"Trained_"+wc+"Weighted.png")
    del canvas
    gc.collect()
