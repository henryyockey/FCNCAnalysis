...
variables=["weight", "topPt", "topEta","l1Pt","l1Eta","mll","drll"]
variablesName=["weight", "topPt", "topEta","l1Pt","l1Eta","mll","drll"]

HistAddress = '/afs/crc.nd.edu/user/r/rgoldouz/FCNC/analysis/hists/'

Samples = ['ANoutput.root']
SamplesName = ['FCNC']
SamplesNameLatex = ['FCNC']
...
for f in range(len(couplingsName)):
    l1=[]
    Files.append(ROOT.TFile.Open(HistAddress + 'tllFCNC' + couplingsName[f] + '.root'))
...
FR = ROOT.TFile.Open(HistAddress + 'tllFCNC_randomRWpoint.root')
FR.Get(namevar).GetSumFit().save('Coup.tex')
for f in range(len(couplingsName)):
    for numvar, namevar in enumerate(variables):
        HH=[]
        HHName=[]
        h2 = Hists[f][numvar].Clone()
        h1 = FR.Get(namevar).Clone()
#        h1.GetSumFit().save("FIT.txt"+namevar)
        text = 'EFTrwgt1_'+couplingsName[f]+'_1' 
#        print couplingsName[f] + ' , ' + str(h1.GetSumFit().evalPoint(ROOT.WCPoint(text))/41530.0)
        wc1 = ROOT.WCPoint(text)
        h1.Scale(wc1)
#        h1.ScaleFits(1/h1.Integral())
        HH.append(h1)
        HHName.append('Reweighted ' + couplingsName[f] + '=1')
        HH.append(h2)
        HHName.append('nominal ' + couplingsName[f] + '=1')
#        drawHistallW(HH,HHName, namevar , couplingsName[f])
