...
channels = ["LLpp", "LLmm", "3LonZ", "3LoffZp", "3LoffZm","4L"]

variables = ['ctp','cpQM','cpt','ctA','ctZ','ctG','cQlM','cQe','ctl','cte','ctlS','ctlT']
...
SamplesName = ["Data", "DY", "tW", "WJets", "ttbar", "diboson", "triboson", "other"]
Hists = [] 

for numyear, nameyear in enumerate(years):
    l0=[]
    Files = []
        ...
        for numch, namech in enumerate(channels):
            ...
            for numreg, namereg in enumerate(regions):
                ...
                for numvar, namevar in enumerate(variables):
                    h= Files[f].Get(namech + '_' + namereg + '_'+ namevar+"TrainedBDT")
    ...
    for numch, namech in enumerate(channels):
        for numreg, namereg in enumerate(regions):
	       ...
	       HHsignal=[]              
               ...
               for wcWeight in variables:
                  text = 'EFTrwgt4_'+wcWeight+'_1.0'
                  wc = ROOT.WCPoint(text)
                  HH[0].Scale(wc)
                  stackPlotsBDT(HH, HHsignal, Samples, namech, namereg, nameyear,namevar,variables[numvar], wcWeight, density=True)
